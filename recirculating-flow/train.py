...
pred_metric = ds.scale_output(cnn.forward(train_grids))
metric_samples = np.array([metric_vals.numpy().flatten(), pred_metric.detach().numpy().flatten()]).T
...
pred_metric = ds.scale_output(cnn.forward(test_grids))
metric_samples = np.array([metric_vals.numpy().flatten(), pred_metric.detach().numpy().flatten()]).T
