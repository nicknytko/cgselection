...
loss_mse_train = helpers.pickle_load('gnn_conv_train_mse_loss.pkl')
loss_mse_test = helpers.pickle_load('gnn_conv_test_mse_loss.pkl')

print('Number of iterations', len(loss_mse_test) - 1)
plt.plot(range(len(loss_mse_train)),  loss_mse_train, 'o-', label='Training set', markersize=2)
plt.plot(range(len(loss_mse_test)), loss_mse_test, 'o-', label='Testing set', markersize=2)
plt.plot([0, len(loss_mse_train)], [trivial_mse_loss, trivial_mse_loss], '-', label='Trivial Predictor')
...
loss_l1_train = helpers.pickle_load('gnn_conv_train_l1_loss.pkl')
loss_l1_test = helpers.pickle_load('gnn_conv_test_l1_loss.pkl')
plt.plot(loss_l1_train, 'o-', label='Training set', markersize=2)
plt.plot(loss_l1_test, 'o-', label='Testing set', markersize=2)
plt.plot([0, len(loss_l1_train)], [trivial_l1_loss, trivial_l1_loss], '-', label='Trivial Predictor')
plt.xscale('symlog'); plt.yscale('symlog')
...
pred_conv = np.array(helpers.pickle_load('gnn_conv_pred_test.pkl'))
plt.figure()
...
pred_conv = np.array(helpers.pickle_load('gnn_conv_pred_train.pkl'))
plt.figure()
...
errs = np.abs(pred_conv[:,0] - pred_conv[:,1])
errmax = np.argmax(errs)
grids = np.array(helpers.pickle_load('grids.pkl'))
#print(grids[errmax], errs[errmax])
print(pred_conv[errmax])
helpers.display_grid_2d(grids[errmax])
plt.show()
